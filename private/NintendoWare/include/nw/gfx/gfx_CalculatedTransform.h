...
public:
    ...
    enum Flag
    {
        // NOTE: It is expected that the flags defined in Graphics.Scenes.CalculatedTransform.cs on the CreativeStudio side match those defined here.
        // 
        //       When off because of a bit shift, change it such that both are the same.
...
public:
    ...
    : m_Flags(FLAG_DEFAULT)
    {
        // m_TransformMatrix and m_Scale are configured in Setup.
        Setup(bone);
    }
    ...
    void SetTranslate(const math::VEC3& translate)
    {
        ...
        // Called within SetTranslate so unnecessary
        //this->EnableFlags(CalculatedTransform::FLAG_IS_DIRTY);
    }
    ...
    void SetTransform(const math::Transform3& transform)
    {
        ...
        // Called within SetRotateAndTranslate so unnecessary
        //this->EnableFlags(CalculatedTransform::FLAG_IS_DIRTY);
    }
    ...
    bool NormalizeRotateMatrix()
    {
        ...
        {
            // Ortho-normalizes the rotation matrix.

            // Calculate the third row of the rotation matrix with a vector product
    ...
    bool RotateMatrixToQuaternion()
    {
        if (!this->IsEnabledFlags(CalculatedTransform::FLAG_IS_IGNORE_ROTATE))
        {
            // Converts the rotation component of a matrix to a quaternion.
            math::QUAT q;
    ...
    bool QuaternionToRotateMatrix()
    {
        if (!this->IsEnabledFlags(CalculatedTransform::FLAG_IS_IGNORE_ROTATE))
        {
            // Converts a quaternion to the rotation component of a matrix.
            ...
            this->m_TransformMatrix.SetColumn(3, t); // Restore the translation component
            ...
            {
                // The zero vector is the rotation axis of the blend result.
                return false;
            }
