...
public:
    ...
    //
    //
    static NW_INLINE void SetDstColorReferred(bool enabled);
    ...
    // TODO: Shadow and Gas are not supported, so they are handled through implementation based on the graphics runtime.
    //
    enum LutTarget
    {
        LUT_TARGET_D0, //
    ...
    static void ActivateFragmentLightEnabled()
    {
        ...
        // Removes BE because bit [22:16] of 0x1c4 is the lutEnable flag for FR, D0, D, and Refl.
    ...
    static void ActivateViewport(const Viewport& viewport)
    {
        ...
        u32 VIEWPORT_COMMAND[] =
        {
            ...
            (ut::Float31::Float32ToBits31( 2.f / viewport.GetBound().GetWidth() ) << 1),
            nw::ut::Float24::Float32ToBits24( viewport.GetBound().GetHeight()/2.f ),
            (ut::Float31::Float32ToBits31( 2.f / viewport.GetBound().GetHeight() ) << 1),
            0,
            (x16 | (y16 << 16)),
            HEADER_VIEWPORT_XY
        };
...
private:
    ...
    static bool  s_IsDstColorReferred;
...
//----------------------------------------
NW_INLINE void
GraphicsDevice::SetDstColorReferred(bool enabled)
{
    s_IsFrameBufferUpdated = s_IsFrameBufferUpdated || (s_IsDstColorReferred != enabled);
    s_IsDstColorReferred = enabled;
}
...
GraphicsDevice::UpdateFrameBufferCommand()
{
    ...
    // The color buffer read and write settings
    ...
    {
        if (s_IsDstColorReferred || (s_ColorMask != COLOR_MASK_ALL))
        {
            s_FrameBufferCommand[COLOR_READ_MASK_INDEX] = COLOR_ACCESS_ENABLE;
        }
    ...
    {
        // The depth buffer read and write settings
        ...
        // The stencil buffer read and write settings
...
GraphicsDevice::ActivateLutLoadSetting(LutTarget target)
{
    ...
    int targetReg = 0;
    // The spot and distance attenuation are set in the register of target+1.
...
GraphicsDevice::ActivatePolygonOffset()
{
    ...
    // In the DMP documentation, when wScale is enabled, the specifications are such that PolygonOffset will not work but since there is a demand for PolygonOffset even during W-Buffering, set it the same as Z-Buffering.
    // 
    if (s_PolygonOffsetEnabled)
    {
        ...
        {
            // As a result of the precision limitations for 24-bit floating point fractions (16-bit mantissa), it is necessary to multiply by a constant so there is no precision problem, to erase errors in calculation.
            // 
            // Multiply by 128.0f using a depth of about 0.5f as a basis.
