...
#ifndef NW_GFX_PARTICLESHAPE_H_
#define NW_GFX_PARTICLESHAPE_H_
...
public:
    ...
    struct VertexAttribute
    {
        ...
        bool m_IsStream;
        u8* m_Stream[2];
        void* m_CommandPtr[2]; //< The starting address for this data in the command
    };
    ...
    //
    static int AddVertexStreamSize(
        u32 formatType,
        int dimension,
        int capacity,
        int prevSize);
    ...
    //
    static int AddVertexParamSize(
        u32 formatType,
        int dimension,
        int prevSize);

    //
    VertexAttribute* AddVertexStream(
        s32 usage,
        u32 formatType,
        int dimension,
        int capacity,
    ...
    //
    VertexAttribute* AddVertexParam(
        s32 usage,
        u32 formatType,
        int dimension,
        f32* parameters,
    ...
    {
        return ResDynamicCast<ResParticleShape>( this->GetResSceneObject() );
    }
    ...
    void SetVertexParameter(int index, ParticleBuffer side, const f32* ptr)
    {
        ...
        u32* target = reinterpret_cast<u32*>(
            this->m_VertexAttribute[index].m_CommandPtr[side]);
...
private:
    //
    //
    VertexAttribute* AddVertexAttribute(
        s32 usage,
        u32 formatType,
        u8 dimension,
        bool isStream,
    ...
    static const int bufferAlignment = 4; // The alignment needed in the stream
    int m_Capacity;
    bool m_BufferSide;
