...
inline void MaterialState::ActivateDepthOperation(const ResDepthOperation depthOperation)
{
    ...
    internal::NWUseCmdlist<sizeof(depthOperation.ref().m_CommandBuffer)>(&depthOperation.ref().m_CommandBuffer[0]);
}
...
inline void MaterialState::ActivateStencilOperation(const ResStencilOperation stencilOperation)
{
    ...
    internal::NWUseCmdlist<sizeof(stencilOperation.ref().m_CommandBuffer)>(&stencilOperation.ref().m_CommandBuffer[0]);
}
...
inline void MaterialState::ActivateBlendOperation(const ResBlendOperation blendOperation)
{
    ...
    internal::NWUseCmdlist<sizeof(blendOperation.ref().m_CommandBuffer)>(blendOperation.ref().m_CommandBuffer);
}
...
inline void MaterialState::ActivateShadingParameters(const SceneEnvironment& sceneEnvironment,const ShaderProgram* shaderProgram,const ResMaterial material)
{
    ...
    if (ut::CheckFlag(shaderProgram->GetActiveDescription().GetFlags(),
...
inline void MaterialState::ActivateRasterization(const ResRasterization rasterization)
{
    ...
    internal::NWUseCmdlist<sizeof(rasterization.ref().m_CommandBuffer)>(rasterization.ref().m_CommandBuffer);
}
...
{
    internal::NWUseCmdlist<sizeof(fragmentShader.ref().m_BufferCommand)>(&(fragmentShader.ref().m_BufferCommand[0]));
    ...
    for (uint stage = 0; stage < COMBINER_AVAILABLE_COUNT; stage++)
    {
        ...
        internal::NWUseCmdlist<sizeof(textureCombiner.ref().m_CommandBuffer)>(reinterpret_cast<const u32*>(textureCombiner.ref().m_CommandBuffer));
    }
}
...
inline void MaterialState::ActivateAlphaTest(const ResAlphaTest alphaTest)
{
    ...
    internal::NWUseCmdlist<sizeof(alphaTest.ref().m_CommandBuffer)>(&alphaTest.ref().m_CommandBuffer[0]);
}
