...
public:
    ...
    void DeactivateDescription(){
        ...
        mProgramObject = 0;
        mDescription = ResShaderProgramDescription(0);
    }

    const ResShaderProgramDescription GetActiveDescription() const { return mDescription; }


    bool IsParticleShader() const{
        return mDescription.GetGeometryShaderIndex() > 0;
    }
    ...
    void SetVertexUniformBool(int index, bool value) const{
        ...
        if (value){
            mVertexIntUniforms[0] |= 0x1 << index;
        }
        else{
            mVertexIntUniforms[0] &= ~(0x1 << index);
        }
    ...
    void SetGeometryUniformBool(int index, bool value) const{
        NW_ASSERT(0 <= index && index < 16);
        if (value){
            mGeometryIntUniforms[0] |= 0x1 << index;
        }
        else{
            mGeometryIntUniforms[0] &= ~(0x1 << index);
        }
    ...
    void SetVertexUniformInt(int index, s32 value) const{
        ...
        mVertexIntUniforms[2 + index] = value & 0xFF;
    }

    void SetGeometryUniformInt(int index, s32 value) const{
        NW_ASSERT(0 <= index && index < 4);
        mGeometryIntUniforms[2 + index] = value & 0xFF;
    }

    void SetVertexUniformInt(int index, s32 x, s32 y) const{
        NW_ASSERT(0 <= index && index < 4);
        mVertexIntUniforms[2 + index] = (x & 0xFF) | ((y & 0xFF) << 8);
    }

    void SetGeometryUniformInt(int index, s32 x, s32 y) const{
        NW_ASSERT(0 <= index && index < 4);
        mGeometryIntUniforms[2 + index] = (x & 0xFF) | ((y & 0xFF) << 8);
    }

    void SetVertexUniformInt(int index, s32 x, s32 y, s32 z) const{
        NW_ASSERT(0 <= index && index < 4);
        mVertexIntUniforms[2 + index] = (x & 0xFF) | ((y & 0xFF) << 8) | ((z & 0xFF) << 16);
    }

    void SetGeometryUniformInt(int index, s32 x, s32 y, s32 z) const{
        NW_ASSERT(0 <= index && index < 4);
        mGeometryIntUniforms[2 + index] = (x & 0xFF) | ((y & 0xFF) << 8) | ((z & 0xFF) << 16);
    }

    void FlushUniform() const{
        internal::NWUseCmdlist<sizeof(mVertexIntUniforms)>(&mVertexIntUniforms[0]);
        if (mDescription.GetGeometryShaderIndex() >= 0)
        {
            internal::NWUseCmdlist<sizeof(mGeometryIntUniforms)>(&mGeometryIntUniforms[0]);
        }
    }
...
private:
    ...
    ResShaderProgramDescription mDescription;
    GLuint mProgramObject;

    GeometryShaderMode mGeometryShaderMode;

    mutable u32 mVertexIntUniforms[6];
    mutable u32 mGeometryIntUniforms[6];
    mutable u32 mDisableGeometry[2];
};
