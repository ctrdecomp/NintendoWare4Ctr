...
struct TexEnvSetting{
    u8 /*TevMode*/ mModeRgb;
    u8 /*TevMode*/ mModeAlpha;
    u8 /*TevSrc*/ mSrcRgb0;
    u8 /*TevSrc*/ mSrcRgb1;
    u8 /*TevSrc*/ mSrcRgb2;
    u8 /*TevSrc*/ mSrcAlpha0;
    u8 /*TevSrc*/ mSrcAlpha1;
    u8 /*TevSrc*/ mSrcAlpha2;
    u8 /*TevOpRgb*/ mOpRgb0;
    u8 /*TevOpRgb*/ mOpRgb1;
    u8 /*TevOpRgb*/ mOpRgb2;
    u8 /*TevOpAlp*/ mOpAlpha0;
    u8 /*TevOpAlp*/ mOpAlpha1;
    u8 /*TevOpAlp*/ mOpAlpha2;
    u8 /*TevScale*/ mScaleRgb;
    u8 /*TevScale*/ mScaleAlpha;

    void SetTexEnv(GraphicsResource& gres, int i) const{
        internal::GL::SetTevCombineRgb(gres, i, TevMode(mModeRgb));
        internal::GL::SetTevCombineAlpha(gres, i, TevMode(mModeAlpha));
        internal::GL::SetTevSrcRgb(gres,i,TevSrc(mSrcRgb0),TevSrc(mSrcRgb1),TevSrc(mSrcRgb2));
        internal::GL::SetTevSrcAlpha(gres,i,TevSrc(mSrcAlpha0),TevSrc(mSrcAlpha1),TevSrc(mSrcAlpha2));
        internal::GL::SetTevOperandRgb(gres,i,TevOpRgb(mOpRgb0),TevOpRgb(mOpRgb1),TevOpRgb(mOpRgb2));
        internal::GL::SetTevOperandAlpha(gres,i,TevOpAlp(mOpAlpha0),TevOpAlp(mOpAlpha1),TevOpAlp(mOpAlpha2));
        internal::GL::SetTevScaleRgb(gres, i, TevScale(mScaleRgb));
        internal::GL::SetTevScaleAlpha(gres, i, TevScale(mScaleAlpha));
    }
};
...
Material::Material(const res::Material* pRes,const ResBlockSet& resBlockSet){
    ...
    if (mpMem){
        ...
        for (unsigned int i = 0; i < mMemNum.texCoordGen; ++i){
...
void Material::Init(){
    this->mColors[INTERPOLATECOLOR_BLACK] = ut::Color8(DefaultBlackColor);
    this->mColors[INTERPOLATECOLOR_WHITE] = ut::Color8(DefaultWhiteColor);
    this->mColors[2] = DefaultWhiteColor;

    InitMatMemNums(&this->mMemCap);
    InitMatMemNums(&this->mMemNum);
...
void Material::ReserveMem(u8 texMapNum,u8 texSRTNum,u8 texCoordGenNum,u8 tevStageNum,bool allocAlpComp,bool allocBlendMode){
    ...
    if ( mMemCap.texMap < texMapNum
      || mMemCap.texSRT < texSRTNum
      || mMemCap.texCoordGen < texCoordGenNum
      || mMemCap.tevStage < tevStageNum
      || mMemCap.alpComp < alpCompNum
      || mMemCap.blendMode < blendModeNum 
    ){
        if (mpMem){
            Layout::FreeMemory(this->mpMem);
            mpMem = 0;
        ...
        if (mpMem){
            ...
            mMemNum.texSRT = mMemCap.texSRT;
            InitTexSRT(GetTexSRTAry(), this->mMemNum.texSRT);

            mMemNum.alpComp = mMemCap.alpComp;
            if (mMemNum.alpComp)
            {
                *this->GetAlphaComparePtr() = AlphaCompare();
            }

            mMemNum.blendMode = mMemCap.blendMode;
            if (mMemNum.blendMode){
...
const TexSRT* Material::GetTexSRTAry() const{
    return internal::ConvertOffsToPtr<TexSRT>(this->mpMem, CalcOffsetTexSRTAry(this->mMemCap));
}
...
const TexCoordGen* Material::GetTexCoordGenAry() const{
    return internal::ConvertOffsToPtr<TexCoordGen>(this->mpMem, CalcOffsetTexCoordGenAry(this->mMemCap));
}
...
const AlphaCompare* Material::GetAlphaComparePtr() const{
    return internal::ConvertOffsToPtr<AlphaCompare>(this->mpMem, CalcOffsetGetAlphaCompare(this->mMemCap));
}
...
const BlendMode* Material::GetBlendModePtr() const{
    return internal::ConvertOffsToPtr<BlendMode>(this->mpMem, CalcOffsetBlendMode(this->mMemCap));
}
...
const TevStage* Material::GetTevStageAry() const{
    return internal::ConvertOffsToPtr<TevStage>(this->mpMem, CalcOffsetTevStageAry(this->mMemCap));
}
...
void Material::SetTexMapNum(u8 num){
    if (num > 0){
        TexMap *const texMaps = GetTexMapAry();
        for (u32 i = mMemNum.texMap; i < num; ++i){
...
void Material::SetTexSRTNum(u8 num){
    if (num > 0){
        ...
        for (u32 i = mMemNum.texSRT; i < num; ++i){
...
void Material::SetTexCoordGenNum(u8 num){
    if (num > 0){
        ...
        for (u32 i = mMemNum.texCoordGen; i < num; ++i){
            new (&texCoordGens[i]) TexCoordGen();
        }
    }

    mMemNum.texCoordGen = num;
...
void Material::SetTevStageNum(u8 num){
    if (num > 0){
        ...
        for (u32 i = mMemNum.tevStage; i < num; ++i){
            new (&tevStages[i]) TevStage();
        }
    }

    mMemNum.tevStage = num;
}
...
u8 Material::GetColorElement(u32 colorType) const{
    ...
    if (colorType < ANIMTARGET_MATCOLOR_MAX){
        ...
        case 0: 
            return this->mColors[idx].r;
        case 1: 
            return this->mColors[idx].g;
        case 2: 
            return this->mColors[idx].b;
        case 3: 
            return this->mColors[idx].a;
        }
    }
...
void Material::SetColorElement(u32 colorType, u8 value){
    if (colorType < ANIMTARGET_MATCOLOR_MAX){
        ...
        switch ((colorType - ANIMTARGET_MATCOLOR_BUFFER_R) % 4){
        case 0: 
            this->mColors[idx].r = value; break;
        case 1: 
            this->mColors[idx].g = value; break;
        case 2: 
            this->mColors[idx].b = value; break;
        case 3: 
            this->mColors[idx].a = value; break;
        }
    }
}
...
void Material::SetupGraphics(const DrawInfo& drawInfo,u8 alpha,bool bInitFrameTransform){
    ...
    if (!this->GetTextureOnly()){
        ...
        if (mMemNum.texCoordGen > 0){
            u32 i = 0;
            const u32 num = ut::Min(this->mMemNum.texCoordGen, u32(TexMapMax));
            for (; i < num; ++i){
    ...
    if (mMemNum.texSRT > 0){
        const u32 num = ut::Min(ut::Min(this->mMemNum.texSRT, u32(this->GetTexMapNum())), u32(TexMapMax));
        for (u32 i = 0; i < num; ++i){
            ...
            LoadTexCoordMatrix(graphicsResource, i, texMtx.matrix);
        }
    }
    ...
    if (mMemNum.texMap > 0){
        ...
        u32 i = 0;
        const u32 num = ut::Min(this->mMemNum.texMap, u32(TexMapMax));
        for (; i < num; ++i){
            const TexMap& texMap = this->GetTexMap(i);
    ...
    if (!this->GetTextureOnly()){
        ...
        u32 i = 0;
        if (mMemNum.tevStage > 0){
            graphicsResource.SetTexEnvAuto(false);

            for (; i < ut::Min(this->mMemNum.tevStage, (u32)internal::TexEnvUnitMax); ++i){
            ...
            bool setTexEnv = !graphicsResource.GetTexEnvAuto() ||
              graphicsResource.GetNumTexMap() != mMemNum.texMap;

            graphicsResource.SetTexEnvAuto(true, this->mMemNum.texMap);

            switch (mMemNum.texMap){
            case 0:
...
void Material::UnbindAnimation(AnimTransform* pAnimTrans){
    internal::UnbindAnimationLink(&this->mAnimList, pAnimTrans);
}
...
void Material::Animate(){
    for (AnimationList::Iterator it = this->mAnimList.GetBeginIter(); it != this->mAnimList.GetEndIter(); ++it){
        if (it->IsEnable()){
...
AnimationLink* Material::FindAnimationLink(AnimTransform* pAnimTrans){
    return internal::FindAnimationLink(&this->mAnimList, pAnimTrans);
}

AnimationLink* Material::FindAnimationLink(const AnimResource& animRes){
    return internal::FindAnimationLink(&this->mAnimList, animRes);
}
