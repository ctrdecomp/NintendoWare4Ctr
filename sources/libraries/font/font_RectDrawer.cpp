...
const u32 DRAWBEGIN_STATICCOMMAND_BYTES  = 656 + sizeof(sDrawInitCommands) + sizeof(u32) * 2 * 1;
...
const u32 sDrawFinalizeCommands[] ={
...
const u32 DRAWEND_STATICCOMMAND_BYTES = 176 + sizeof(sDrawFinalizeCommands);

}

bool RectDrawer::sStaticInitialized = false;
RectDrawer::VertexBuffer RectDrawer::sVertexBuffer;
...
RectDrawer::RectDrawer():   
    ...
    mUniformMtxIndex(0),
    mUniformDataIndex(0),
    mUniformTextColorIndex(0),
    mUniformAddr(reinterpret_cast<UniformAddress*>(&mUniformAddrBuffer[3])),
    mUniformMtx(reinterpret_cast<math::VEC4*>(&mUniformMtxBuffer[3])),
    mUniformData(reinterpret_cast<math::VEC4*>(&mUniformDataBuffer[3])),
    mUniformTextColor(reinterpret_cast<math::VEC4*>(&mUniformTextColorBuffer[3])),
    mCommandBuffer(NULL),
    mVertexBufferArray(NULL),
    mVertexBufferBaseAddr(0),
    mIsColorDirty(false)
{}
...
void RectDrawer::SetProjectionMtx(const nn::math::MTX44& mtx){
    nn::math::MTX44 *const pDst = reinterpret_cast<nn::math::MTX44*>(&this->mUniformProjBuffer[3]);
    nn::math::MTX44Copy(pDst, &mtx);

    const u32 size = SetUniformCommand(this->mUniformProjBuffer, UNIFORM_PROJ_NUM + 1);
    NW_FONT_RECTDRAWER_ADD_COMMAND(this->mUniformProjBuffer, size);
}

void RectDrawer::SetViewMtxForText(const nn::math::MTX34& mtx){
    nn::math::MTX34 *const pDst = reinterpret_cast<nn::math::MTX34*>(&this->mUniformMtxBuffer[3]);
    nn::math::MTX34Copy(pDst, &mtx);

    mUniformMtxIndex = sizeof(mtx) / sizeof(nn::math::VEC4);
    const u32 size = SetUniformCommand(this->mUniformMtxBuffer, mUniformMtxIndex);
    NW_FONT_RECTDRAWER_ADD_COMMAND(this->mUniformMtxBuffer, size);
    mUniformMtxIndex = 0;
}

void RectDrawer::UseEndCommand(){
    this->mCmdCacheDrawEnd.Use();
}

void RectDrawer::SetParallax(f32 parallax,f32 dLevel,f32 addDist){

    nn::math::VEC4 *const pParallaxData = reinterpret_cast<nn::math::VEC4*>(&this->mUniformProjBuffer[3 + 4 * 4]);
...
void RectDrawer::InitializeStatic(){
    ...
    for (int addr = 0; addr < UNIFORM_ADDR_NUM; ++addr){
        for (int vi = 0; vi < DRAW_VTX_NUM; ++vi){
            sVertexBuffer.position[addr][vi][0] = addr;
            sVertexBuffer.position[addr][vi][1] = vtx[vi];
            sVertexBuffer.position[addr][vi][2] = vtx[vi] % 2 ?  1.f : 0.f;
            sVertexBuffer.position[addr][vi][3] = vtx[vi] / 2 ? -1.f : 0.f;
        }
    }
}

void RectDrawer::AddUniformMtx(){
    const u32 size = this->SetUniformCommand(this->mUniformMtxBuffer, this->mUniformMtxIndex);
    NW_FONT_RECTDRAWER_ADD_COMMAND(this->mUniformMtxBuffer, size);
...
void RectDrawer::InitializeCMD(void* vertexBuffer,void* commandBuffer,const void* shaderBinary,u32 /* size */,bool initVertexBuffer){
    ...
    this->mCmdCacheDrawBegin.Init(this->mCommandBuffer, DrawBeginCommandBytes);
    this->mCmdCacheDrawEnd.Init(ut::AddOffsetToPtr(this->mCommandBuffer, DrawBeginCommandBytes),DRAWEND_STATICCOMMAND_BYTES);
...

    {
       ...
       this->mCmdCacheDrawBegin.Add(commands, sizeof(commands));
    }

    {
        ...
        for (u32 totalCnt = 0; totalCnt < pkgHead.instCount;){
            ...
            this->mCmdCacheDrawBegin.Add(commands, sizeof(commands));
            this->mCmdCacheDrawBegin.Add(&progCodes[totalCnt + 1], sizeof(u32) * (writeCnt - 1));
            this->mCmdCacheDrawBegin.RoundUp(8);
            totalCnt += writeCnt;
        }
...

    {
        ...
        for (u32 totalCnt = 0; totalCnt < pkgHead.swizzleCount; ){
            ...
            for (int i = 1; i < writeCnt; ++i){
                this->mCmdCacheDrawBegin.Add(&swDatas[totalCnt + i].value, sizeof(u32));
            }
...

    {
        ...
        for (int i = 0; i < exeHead.setupCount; ++i){
            ...
            mCmdCacheDrawBegin.Add(commands, sizeof(commands));
        }
    }

    const uptr phyArrayBuffer = nngxGetPhysicalAddr(reinterpret_cast<uptr>(mVertexBufferArray));

    const u32 vboBaseAddr = math::RoundDown(phyArrayBuffer, 16);

    AddVertexAttributeArrayCommand(this->mCmdCacheDrawBegin, vboBaseAddr, phyArrayBuffer, vtxAttrNum);
...

    {
...

        {
            ...
            this->mCmdCacheDrawBegin.Add(commands, sizeof(commands));
            this->mCmdCacheDrawBegin.Add(reinterpret_cast<u32*>(outmaps[1]), sizeof(u32) * (OutRegNum - 1));
        }
    ...
    this->mCmdCacheDrawBegin.Add(sDrawInitCommands, sizeof(sDrawInitCommands));

    this->mVertexBufferBaseAddr = vboBaseAddr;

    {
        void *const eleArrayBuf = ut::AddOffsetToPtr(this->mVertexBufferArray, sizeof(sVertexBuffer.position));
    ...
    AddVertexAttributeArrayCommand(this->mCmdCacheDrawEnd, vboBaseAddr, phyArrayBuffer, vtxAttrNum);

    this->mCmdCacheDrawEnd.Add(sDrawFinalizeCommands, sizeof(sDrawFinalizeCommands));

    {
        InitUniformBuffer(this->mUniformProjBuffer, UNIFORM_PROJ_START);

        SetParallax(0.0f, 0.0f, 0.0f);

        InitUniformBuffer(this->mUniformAddrBuffer, UNIFORM_ADDR_START);
        InitUniformBuffer(this->mUniformMtxBuffer, UNIFORM_MTX_START);
        InitUniformBuffer(this->mUniformDataBuffer, UNIFORM_DATA_START);
    }
}
