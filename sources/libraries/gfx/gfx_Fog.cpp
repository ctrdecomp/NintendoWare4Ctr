...
Fog* Fog::DynamicBuilder::Create(nw::os::IAllocator* allocator){
    ...
    Fog* fog = new(memory) Fog(allocator, resource, mDescription);
...
ResFogData* Fog::CreateResFog(nw::os::IAllocator* allocator, const char* name /* = NULL */){
    ...
    resFog->mHeader.revision = ResFog::BINARY_REVISION;
    resFog->mHeader.signature = ResFog::SIGNATURE;

    resFog->mUserDataDicCount = 0;
    resFog->toUserDataDic.set_ptr(NULL);

    resFog->toName.set_ptr(AllocateAndCopyString(name, allocator, MAX_NAME_LENGTH));

    resFog->mChildrenTableCount = 0;
    resFog->toChildrenTable.set_ptr(NULL);
    resFog->mAnimGroupsDicCount = NULL;
    ...
    resFog->mTransform = math::Transform3(scale, rotate, translate);
    resFog->mWorldMatrix = math::MTX34::Identity();
    ...
    fogSampler->mCommandCacheTableCount = NW_FOG_TABLE_COMMAND_SIZE;
...
void Fog::Update(const Camera* camera){
    ...
    ResFogUpdaterData* updaterData = mUpdaterCache.Get();
    if (!ut::FloatEqualsWeak(mNear, near) ||
        !ut::FloatEqualsWeak(mFar, far) ||
        !ut::FloatEqualsWeak(mWScale, wscale) ||
        !ut::FloatEqualsWeak(updaterData->mDensity, fogUpdater.GetDensity()) ||
        !(updaterData->mFogUpdaterType == fogUpdater.GetFogUpdaterType()) ||
        !ut::FloatEqualsWeak(updaterData->mMaxFogDepth, fogUpdater.GetMaxFogDepth()) ||
        !ut::FloatEqualsWeak(updaterData->mMinFogDepth, fogUpdater.GetMinFogDepth())){
        mNear = near;
        mFar = far;
        mWScale = wscale;
        updaterData->mDensity = fogUpdater.GetDensity();
        updaterData->mFogUpdaterType = fogUpdater.GetFogUpdaterType();
        updaterData->mMaxFogDepth = fogUpdater.GetMaxFogDepth();
        updaterData->mMinFogDepth = fogUpdater.GetMinFogDepth();
...
void Fog::SetupFogSampler(ResImageLookupTable fogSampler, ResFogUpdater fogUpdater, const math::MTX44& inverseProjectionMatrix){
    ...
    for (int i = 0; i <= HALF_TABLE_SIZE; ++i){
        if (mWScale == 0.0f){
            ...
            viewPos.z = ((f32)i / (f32)HALF_TABLE_SIZE) * (mFar - mNear) + mNear;
        }
...
Result Fog::Initialize(nw::os::IAllocator* allocator){
    ...
    if (resFogUpdater != NULL){
        mUpdaterCache = ResUpdaterPtr(resFogUpdater, ResFogUpdaterDataDestroyer(allocator));
    }
    else{
...
Result Fog::CreateOriginalValue(nw::os::IAllocator* allocator){
    ...
    mOriginalValue = ResFog(originalValue);
...
void Fog::BindAnim(AnimGroup* animGroup){
    ...
    for (int memberIdx = 0; memberIdx < animMemberCount; ++memberIdx){
        ...
        case anim::ResAnimGroupMember::OBJECT_TYPE_FOG:{
                ...
                u8* originalValue = reinterpret_cast<u8*>(mOriginalValue.ptr());
