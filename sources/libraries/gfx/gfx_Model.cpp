...
void Model::BindVisibilityAnim(AnimGroup* animGroup){
    ...
    for (int memberIdx = 0; memberIdx < animMemberCount; ++memberIdx){
        ...
        case anim::ResAnimGroupMember::OBJECT_TYPE_MODEL:{
                ...
                animGroup->SetOriginalValue(memberIdx, &mOriginalVisibility);
            }
            break;
        case anim::ResAnimGroupMember::OBJECT_TYPE_MESH:{
                ...
                animGroup->SetTargetPtr(memberIdx, target);
                animGroup->SetOriginalValue(memberIdx, &mMeshOriginalVisibilities[meshIndex]);
            }
            break;
        case anim::ResAnimGroupMember::OBJECT_TYPE_MESH_NODE_VISIBILITY:{
                ...
                animGroup->SetTargetPtr(memberIdx, target);
                animGroup->SetOriginalValue(memberIdx, &mMeshNodeOriginalVisibilities[visibilityIndex]);
            }
            break;
        default:{
...
Result Model::CreateAnimGroups(nw::os::IAllocator* allocator){
    ...
    for (int animGroupIdx = 0; animGroupIdx < animGroupCount; ++animGroupIdx){
        ...
        if (targetType == anim::ResGraphicsAnimGroup::TARGET_TYPE_MATERIAL && mSharingMaterial){
            continue;
        }
        ...
        if (targetType == anim::ResGraphicsAnimGroup::TARGET_TYPE_MATERIAL ||
            ...
            case anim::ResGraphicsAnimGroup::TARGET_TYPE_MATERIAL:{
                    ...
                    mMaterialAnimGroup = animGroup;
                    mMaterialAnimBindingIndex = animGroupIdx;
                }
                break;
            ...
            case anim::ResGraphicsAnimGroup::TARGET_TYPE_VISIBILITY:{
                    ...
                    animBinding->SetAnimGroup(animGroupIdx, animGroup);
                    mVisibilityAnimGroup = animGroup;
                    mVisibilityAnimBindingIndex = animGroupIdx;
                }
                break;
...
Result Model::CreateMaterials(nw::os::IAllocator* allocator){
    ...
    if (mSharingMaterial){
        NW_ASSERTMSG(mBufferOption == 0, "In the case of using a shared material, it can not use buffer option.");
        mBufferOption = mDescription.sharedMaterialModel->GetBufferOption();

        int materialCount = mDescription.sharedMaterialModel->GetMaterialCount();
        ...
        mMaterials.Reset(memory, materialCount, allocator);

        for (int i = 0; i < materialCount; i++){
            mMaterials.PushBackFast(mDescription.sharedMaterialModel->GetMaterial(i));
        }
    }
    else{
        ResModel resModel = GetResModel();
        ...
        if (materialCount != 0){
            ...
            mMaterials.Reset(memory, materialCount, allocator);
            s32 bufferCount = (mBufferOption == 0x0) ? 0 : 1;
            ...
            for (int i = 0; i < materialCount; i++){
                ...
                mMaterials.PushBackFast(material);
            }
        }
    }
...
Result Model::CreateResMeshes(nw::os::IAllocator* allocator){
    ...
    if (meshesCount != 0){
        ...
        mMeshBuffers = ResMeshArray(meshOffsets, meshesCount);

        mOriginalVisibility = resModel.IsVisible();
        ...
        mMeshOriginalVisibilities.Reset(memory, meshesCount, allocator);

        for (int i = 0; i < meshesCount; ++i){
            mMeshOriginalVisibilities.PushBackFast(mMeshBuffers[i].IsVisible());
        }
    }
...
Result Model::CreateResMeshNodeVisibilities(nw::os::IAllocator* allocator){
    ...
    if (visibilitiesConut != 0){
        ...
        mMeshNodeVisibilityBuffers.Reset(buffer, visibilitiesConut, allocator);

        for (int i = 0; i < visibilitiesConut; ++i){
            mMeshNodeVisibilityBuffers.PushBackFast(*(resModel.GetMeshNodeVisibilities(i).ptr()));
        }
        ...
        mMeshNodeOriginalVisibilities.Reset(memory, visibilitiesConut, allocator);

        for (int i = 0; i < visibilitiesConut; ++i){
            mMeshNodeOriginalVisibilities.PushBackFast(resModel.GetMeshNodeVisibilities(i).IsVisible());
        }
    }
...
Result Model::CreateCallbacks(nw::os::IAllocator* allocator){
    ...
    if (mDescription.isFixedSizeMemory){
        if (mDescription.maxCallbacks == 0){
            mPreRenderSignal = RenderSignal::CreateInvalidateSignal(allocator);
            mPostRenderSignal = RenderSignal::CreateInvalidateSignal(allocator);
        }
        else{
            mPreRenderSignal = RenderSignal::CreateFixedSizedSignal(mDescription.maxCallbacks, allocator);
            mPostRenderSignal = RenderSignal::CreateFixedSizedSignal(mDescription.maxCallbacks, allocator);
        }
    }
    else{
        mPreRenderSignal = RenderSignal::CreateVariableSizeSignal(allocator);
        mPostRenderSignal = RenderSignal::CreateVariableSizeSignal(allocator);
    }

    if (mPreRenderSignal == NULL || mPostRenderSignal == NULL){
        result |= Result::MASK_FAIL_BIT;
    }
...
Result Model::CreateMaterialActivator(nw::os::IAllocator* allocator){
    ...
    if (!mSharingMaterial){
        IMaterialActivator* activator =
            (mDescription.bufferOption == 0 || mDescription.bufferOption == FLAG_BUFFER_SCENE_ENVIRONMENT) ?
        ...
        if (activator != NULL){
            mMaterialActivator.Reset(activator);
        }
        else{
            result |= Result::MASK_FAIL_BIT;
        }
...
void Model::InvalidateRenderKeyCache(){
    ResMeshArray::iterator end = mMeshBuffers.end();
    for (ResMeshArray::iterator mesh = mMeshBuffers.begin(); mesh != end; ++mesh){
