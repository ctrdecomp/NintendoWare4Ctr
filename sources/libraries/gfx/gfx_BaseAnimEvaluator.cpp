...
void BaseAnimEvaluator::ResetMember(int memberIdx){
    NW_NULL_ASSERT(this->mAnimGroup);

    if (!this->mAnimGroup->HasOriginalValue()){
        return;
    }

    anim::ResGraphicsAnimGroup data = this->mAnimGroup->GetResGraphicsAnimGroup();
    NW_ASSERT(data.IsValid());

    const anim::ResAnimGroupMember resAnimGroupMember = this->mAnimGroup->GetResAnimGroupMember(memberIdx);
    resAnimGroupMember.SetValueForType(this->mAnimGroup->GetTargetObject(memberIdx),this->mAnimGroup->GetOriginalValue(memberIdx));
