// Filename: gfx_SceneInitializer.cpp
//
...
SceneInitializer* SceneInitializer::Builder::Create(os::IAllocator* allocator)
{
    ...
    NW_ASSERTMSG(this->m_Description.materialIdGenerator != NULL, "SceneInitializer need to set an IMaterialIdGenerator instance.");

    SceneInitializer* initializer = new(memory) SceneInitializer(allocator, this->m_Description);
...
{
    if (this->m_MaterialIdGenerator.Get() != NULL)
    {
        ...
        for (gfx::MaterialArray::iterator material = model->GetMaterials().first;
            ...
            ++material)
            {
            this->m_MaterialIdGenerator.Get()->Accept(*material);
        }
    }
}
...
{
    this->m_MaterialIdGenerator.Get()->Generate();
}
