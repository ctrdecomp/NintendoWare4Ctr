...
const float CurveAdshr::VOLUME_INIT = -90.4f;

const s16 CurveAdshr::DecibelSquareTable[DECIBEL_SQUARE_TABLE_SIZE] =
{
...
void CurveAdshr::Update(int msec)
{
    ...
    case STATUS_DECAY:
    {
        const f32 sustainDecay = CalcDecibelSquare(m_Sustain);
...
{
    NW_MINMAX_ASSERT(sustain, 0, 127);

    m_Sustain = static_cast<u8>(sustain);
}
...
f32 CurveAdshr::CalcRelease(int release)
{
    NW_MINMAX_ASSERT(release, 0, 127);
...
{
    NW_MINMAX_ASSERT(scale, 0, CALC_DECIBEL_SCALE_MAX);
